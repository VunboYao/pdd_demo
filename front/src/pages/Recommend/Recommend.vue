  ...
  export default {
    ...
    methods: {
      ...
      async dealWatchBtnClick(goods) {
        // 1. 发送请求
        let result = await addGoods2Car(this.userInfo.id,goods.goods_id,goods.short_name,goods.thumb_url,goods.price,goods.sales_tip);
        Indicator.open(result.message);
        console.log(result);
        setTimeout(() => {
          Indicator.close();
        },500)
