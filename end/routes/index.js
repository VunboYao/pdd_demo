    ...
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) { // 3.1 hasGoods
                ...
                let add_sql = "INSERT INTO lk_pdd_cart(goods_id,goods_name, thumb_url, price, buy_count,is_pay,sales_tip) VALUES (?,?,?,?,?,?,?)";
                let sql_params = [goods_id, goods_name, thumb_url, price, buy_count, is_pay, sales_tip];
...
/* 删除购物车数据2 */
router.post('/api/del_car_goods_group', (req, res) => {
    // 1. 获取参数
    let user_id = req.body.user_id;
    let goods_ids = req.body.goods_ids.toString();
    if (!req.session.userId) {
        res.json({err_code: 0, message: '请先登陆'})
        return;
    }
    //1.1 数据库查询语句
    let sqlStr = "DELETE FROM lk_pdd_cart WHERE goods_id in" + '(' + goods_ids + ')';
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '数据发送失败'})
        } else {
...
router.get('/api/recommend', function (req, res, next) {
    ...
    for (let i = 0; i < recommendArr.length; i++) {
       ...
       /* temp_arr.push(oldItem.hd_thumb_url)
        temp_arr.push(oldItem.image_url)*/
        temp_arr.push(oldItem.price.substring(1))
       /* temp_arr.push(oldItem.normal_price)
        temp_arr.push(oldItem.market_price)*/
        temp_arr.push(oldItem.sales_tip)
        // temp_arr.push(oldItem.hd_url)
    ...
    // 3. 批量插入数据库表
    // 3.1 数据库查询语句
    let sqlStr = "INSERT INTO pdd_recommend (`short_name`, `thumb_url`, `price`, `sales_tip`) VALUES ?";
    //3.2执行语句
    conn.query(sqlStr, [temp_arr_all], (error, results, fields) => {
        if (error) {
            console.log('INSERT ERROR - ', error.message);
